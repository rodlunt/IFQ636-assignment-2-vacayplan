...
Short-lived feature branches and pull requests let the three of us work in parallel while keeping main deployable, with every merge reviewed by at least one other member. A post-merge review of PR #66 caught a frontend regression both test suites had missed, showing that backend unit tests do not guarantee API contract safety. Merge conflicts proved similarly productive: resolving conflict #2, between Rodney's Chain of Responsibility entry and Lance's Facade branch, forced the implementation references up to date and left the documentation more accurate than either branch alone.

Collaboration widened the design too: the weather adapter gave the Adapter pattern a natural home the existing code lacked, and the pipeline was split into pr-checks.yml and ci.yml once a deploy-time gate proved insufficient as the sole test gate.

We verified performance against NFR-01 with a load test on the live EC2 deployment (GET /api/trips, authenticated). At normal load (3 users) the median was 25ms with no errors, well inside the 500ms target; under stress (10 users) the median held at 30ms but the 95th-percentile tail rose to 892ms (Figs 8.1-8.2). Every request succeeded and throughput stayed flat near 52 requests per second, pointing to the single EC2 node, not the code, as the bottleneck. Because the API is stateless and authenticates through self-contained JWTs, it scales horizontally (NFR-11): further instances can sit behind a load balancer without session affinity, and indexing trip queries on userId would trim the database time dominating the tail (Queensland University of Technology, 2026b). The single-instance deployment meets the median target with a clear, low-risk path to higher concurrency.
...
Overall, VacayPlan is a working full-stack trip-planning application built on documented design patterns and OOP principles, tested with unit and Postman API checks, and deployed on every push through an automated CI/CD pipeline. 
